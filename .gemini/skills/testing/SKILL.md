...
- **Fixed-Width Synchronization**: To ensure data points (Name, Age, Team, Time) line up perfectly in a straight vertical line across multiple rows, use **fixed widths** (in `pt` or `px`) for these columns instead of `flex: 1`. This prevents horizontal shifting when content lengths vary.
- **Top-Alignment Rule**: When allowing text wrapping (e.g., for long names), use `align-items: flex-start` for entry rows. This ensures that columns remain horizontally aligned at the start of each row even if one field spans multiple lines.
- **Dynamic Overflow Pattern**: For horizontal elements that should fill the remaining space (e.g., DQ entry lines), use `flex: 1` on the container and `width: 100%` on the internal drawing element. This ensures the line extends to the edge of the column but never overflows into gutters or adjacent columns.
