...
class ReportGenerator:
    ...
    def generate_psych_sheet(self, output_path):
        ...
        elements.append(
            Paragraph(self.data.get("meetName", "Unknown Meet"), self.custom_styles["MeetInfo"])
        )
        elements.append(
            Paragraph(
                f"{self.data.get('meetStart', '')} - {self.data.get('meetEnd', '')}",
                self.custom_styles["MeetInfo"],
            )
        )
        ...
        for session in self.data.get("sessions", []):
            for event in session.get("events", []):
                elements.append(
                    Paragraph(
                        f"Event {event['eventNum']}: {event['eventDesc']}",
                        self.custom_styles["EventHeader"],
                    )
                )
                ...
                t = Table(
                    table_data,
                    colWidths=[0.5 * inch, 2.5 * inch, 0.5 * inch, 2.5 * inch, 1.0 * inch],
                )
                t.setStyle(
                    TableStyle(
                        [
    ...
    def generate_meet_entries(self, output_path, team_filter=None):
        ...
        for rec in records:
            ...
            if rec["name"] != current_athlete:
                ...
                elements.append(
                    Paragraph(f"{current_athlete} (Age: {rec['age']})", self.styles["Heading3"])
                )

            elements.append(
                Paragraph(
                    f"Event {rec['eventNum']}: {rec['eventDesc']} - {rec['seedTime']}",
                    self.styles["Normal"],
                )
            )
    ...
    def generate_lineup_sheets(self, output_path):
        ...
        for session in self.data.get("sessions", []):
            for event in session.get("events", []):
                elements.append(
                    Paragraph(f"Lineup for {event['eventDesc']}", self.custom_styles["EventHeader"])
                )

                table_data = [["Heat", "Lane", "Name", "Team"]]
                entries = sorted(
                    event.get("entries", []), key=lambda x: (x.get("heat", 0), x.get("lane", 0))
                )
    ...
    def generate_meet_results(self, output_path):
        ...
        for session in self.data.get("sessions", []):
            for event in session.get("events", []):
                elements.append(
                    Paragraph(f"Results: {event['eventDesc']}", self.custom_styles["EventHeader"])
                )
    ...
    def generate_meet_program(self, output_path):
        ...
        frame2 = Frame(
            left_margin + frame_width + 0.2 * inch,
            bottom_margin,
            frame_width,
            frame_height,
            id="col2",
        ...
        template = PageTemplate(
            id="TwoCol", frames=[frame1, frame2], onPage=self._header_footer_program
        )
        ...
        style_entry = ParagraphStyle(
            "ProgramEntry", parent=self.styles["Normal"], fontSize=9, leading=10
        )
        ...
        for session in self.data.get("sessions", []):
            ...
            for event in events:
                ...
                # Title: Event # - Desc
                event_block.append(
                    Paragraph(f"Event {event['eventNum']}  {event['eventDesc']}", style_event_title)
                )
                ...
                if is_relay:
                    event_block.append(
                        Paragraph("Lane  Team           Relay    Seed Time", style_header_row)
                    )
                else:
                    event_block.append(
                        Paragraph(
                ...
                for entry in entries:
                    ...
                    if heat != current_heat:
                        ...
                        event_block.append(
                            Paragraph(
                                f"Heat {heat} of {event.get('numHeats', '?')}", style_entry_bold
                            )
                        )
