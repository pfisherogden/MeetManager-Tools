...
import type { Score, EventScore, Entry } from "@/lib/swim-meet-types"
...
const getMeetName = (meetId: string) => meetId
...
export function ScoresManager({ initialScores, initialEventScores = [] }: ScoresManagerProps) {
    const [data, setData] = useState<Score[]>(initialScores)

    // Flatten event scores for table
    const eventRows = useMemo(() => initialEventScores.flatMap(ev =>
        ev.entries.map(e => ({
        ...
        if (a.eventName === b.eventName) return (a.place || 999) - (b.place || 999);
    ...
    const eventColumns = useMemo<Column<any>[]>(() => {
        const uniqueEvents = Array.from(new Set(eventRows.map(r => r.eventName))).sort()
        const uniqueTeams = Array.from(new Set(eventRows.map(r => r.teamName))).sort()
        const uniqueRanks = Array.from(new Set(eventRows.map(r => r.place))).sort((a, b) => (Number(a) || 999) - (Number(b) || 999))

        return [
            { key: "eventName", label: "Event", editable: false, width: "w-48", filterVariant: "faceted", options: uniqueEvents },
