...
test.describe('Champs Dataset Journey', () => {
  test('should correctly process and display Champs 2025 dataset', async ({ page }) => {
    ...
    if (await existingRow.count() === 0) {
        ...
        await expect(page.getByText('Dataset uploaded successfully')).toBeVisible({ timeout: 30000 });
    }
    ...
    await expect(page.locator('tr').filter({ hasText: 'config.json' })).not.toBeVisible();
    ...
    await page.getByRole('heading', { name: 'Meet Program (PDF)', exact: true }).click();
    await page.getByRole('button', { name: 'Add to Pack' }).click();
    
    // Select Lineup Sheets Card
    await page.getByRole('heading', { name: 'Lineup Sheets', exact: true }).click();
    await page.getByRole('button', { name: 'Add to Pack' }).click();

    // Click Generate Bundle ZIP in the Pack summary section
    const generateZipBtn = page.getByRole('button', { name: /Generate Bundle ZIP/i });
    await expect(generateZipBtn).toBeVisible({ timeout: 20000 });
    await generateZipBtn.click();
    
    await expect(page.getByText('Bundle generated successfully', { exact: false })).toBeVisible({ timeout: 60000 });
  });
});
