...
test.describe('Ingestion and Admin Journey', () => {
  test('should allow navigating to Admin and uploading a dataset', async ({ page }, testInfo) => {
  ...
  test('should navigate to Reports and generate a Meet Program PDF', async ({ page }) => {
    ...
    const meetProgramCard = page.locator('div').filter({ hasText: /^Meet Program/ }).first();
    const generateButton = meetProgramCard.getByRole('button', { name: /Generate/i });
