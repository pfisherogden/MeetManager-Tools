...
test("Tauri Desktop App smoke test & performance check", async ({ tauriApp }) => {
	...
	const defaultUrl = process.platform === "win32" ? "http://tauri.localhost/" : "tauri://localhost/";

	// Measure initial load time (including JVM initialization)
	const initialStart = Date.now();
	await page.goto(defaultUrl);
	await expect(page.locator("text=Dataset Management")).toBeVisible({ timeout: 30000 });
