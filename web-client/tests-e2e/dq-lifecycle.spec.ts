...
test.describe("Disqualification Lifecycle", () => {
	...
	test.describe("Full DQ-to-PDF Lifecycle", () => {
		test("should reflect a submitted DQ in the final Results PDF", async ({
			...
			await judgePage.getByTestId("heat-item-1").click();
			...
			await adminPage.getByText(/Review DQs/i).click();
			await expect(adminPage.locator("table")).toContainText("1A");

			// 4. Generate Results PDF and verify
