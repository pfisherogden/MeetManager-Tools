...
test.describe("Disqualification Lifecycle", () => {
	...
	test.beforeAll(async ({ browser }) => {
		...
		await judgeContext.addCookies([{ name: "x-user-id", value: userId, domain: "localhost", path: "/" }]);
		await volunteerContext.addCookies([{ name: "x-user-id", value: userId, domain: "localhost", path: "/" }]);
	});
	...
	test("Full DQ Journey: Publish -> Submit -> Sync -> Verify", async () => {
		...
		await expect(volunteerPage.getByRole("heading", { name: "Dashboard" })).toBeVisible();
		...
		await judgePage.getByText(/Event 3/i).first().click(); 
		...
		await expect(volunteerPage.locator("tr").filter({ hasText: "1A" })).toBeVisible();
		...
		await judgePage.getByText(/Event 1/i).first().click(); // Relay
		await judgePage.getByText(/Heat 1/i).first().click();
		...
		await expect(volunteerPage.locator("tr").filter({ hasText: "7Q" })).toBeVisible();
