...
- **Jest Testing & Large JSON**: When `babel-jest` evaluates a large `.json` file imported by a test or application file, the Jest abstract syntax tree (AST) construction can hang indefinitely (memory spike). To resolve this, exclude JSON files from Jest transformation via `transformIgnorePatterns: ["\\\\.json$"]` in `package.json` so the payload loads natively in Node instead of bloating the Babel pipeline.