    ...
    def ValidateMeet(self, request, context):
        ...
        try:
            ...
            for e in events:
                ...
                if e_ptr:
                    events_map[e_ptr] = {
                        ...
                        "dist": self._safe_int(self._get_field(e, ["event_dist", "Event_dist"])),
                        "stroke": self._get_field(e, ["event_stroke", "Event_stroke"]),
                    }

            # Map entries by athlete
            athlete_entries: dict[int, list[int]] = collections.defaultdict(list)
            ...
            for entry in entries:
                ...
                ev_score = self._safe_float(self._get_field(entry, ["ev_score", "Ev_score"]))
                scr_stat = self._safe_int(self._get_field(entry, ["scr_stat", "Scr_stat"]))
                ...
                if fin_time > 0:
                    actualseed_time = self._safe_float(self._get_field(entry, ["actualseed_time", "actual_seed"]))
            ...
            relays = self._get_table(cache, "relay")
            for r in relays:
                r_heat = self._safe_int(self._get_field(r, ["fin_heat", "Fin_heat"]))
                r_lane = self._safe_int(self._get_field(r, ["fin_lane", "Fin_lane"]))
                if r_heat > 0 and r_lane > 0:
                    r_time = self._safe_float(self._get_field(r, ["fin_time", "Fin_time"]))
                    r_stat = str(self._get_field(r, ["fin_stat", "Fin_stat"]) or "").strip().upper()
                    if r_time == 0 and r_stat not in ["Q", "R"]:
                        t_id = self._safe_int(self._get_field(r, ["team_ptr", "team_no", "Team_ptr", "Team_no"]))
                        team_name = teams_map.get(t_id, {}).get("name", f"Team #{t_id}")
                        team_ltr = str(self._get_field(r, ["team_ltr", "Team_ltr"]) or "").strip()
                        evt_id = self._safe_int(self._get_field(r, ["event_ptr", "Event_ptr"]))
            ...
            for evt_id, times_list in event_times.items():
                ...
                for t, heat, lane, name in times_list:
                    time_to_entries[t].append((heat, lane, name))
                for t, entries_at_time in time_to_entries.items():
                    ...
                    if len(heats) > 1:
                        ...
                        findings.append(
                            pb2.ValidationFinding(
                                ...
                                message=f"{evt_desc}: Time {t:.2f}s matches exactly in different heats: {', '.join(swimmers_info)}.",
